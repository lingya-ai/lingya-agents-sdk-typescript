...
export const BASE_PATH = "https://tenant.example.com".replace(/\/+$/, "");
...
export function mapValues<T>(data: { [key: string]: T }, fn: (item: T) => T): { [key: string]: T } {
    return Object.keys(data).reduce((result, key) => ({ ...result, [key]: fn(data[key]) }), {});
}
